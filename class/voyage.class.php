...
class Voyage extends CommonObject
{
	 ...
	 */
	public $module = 'clienjoyholidaysv2';
	 ...
	 */
	public $table_element = 'clienjoyholidaysv2_voyage';
	...
	/**
	 * @var string String with name of icon for voyage. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'voyage@clienjoyholidaysv2' if picto is file 'img/object_voyage.png'.
	 */
	...
	public $fields=array(
		...
		'label' => array('type'=>'varchar(255)', 'label'=>'Label', 'enabled'=>'1', 'position'=>30, 'notnull'=>0, 'visible'=>1, 'alwayseditable'=>'1', 'searchall'=>1, 'css'=>'minwidth300', 'cssview'=>'wordbreak', 'validate'=>'1',),
		'amount' => array('type'=>'price', 'label'=>'Amount', 'enabled'=>'1', 'position'=>40, 'notnull'=>0, 'visible'=>1, 'default'=>'null', 'isameasure'=>'1', 'validate'=>'1',),
		'fk_soc' => array('type'=>'integer:Societe:societe/class/societe.class.php:1:status=1 AND entity IN (__SHARED_ENTITIES__)', 'label'=>'ThirdParty', 'enabled'=>'1', 'position'=>50, 'notnull'=>-1, 'visible'=>1, 'index'=>1, 'css'=>'maxwidth500 widthcentpercentminusxx', 'csslist'=>'tdoverflowmax150', 'validate'=>'1',),
	...
	//  */
	// public $table_element_line = 'clienjoyholidaysv2_voyageline';
	...
	//  */
	// protected $childtablesoncascade = array('clienjoyholidaysv2_voyagedet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->clienjoyholidaysv2->voyage->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->clienjoyholidaysv2->voyage->write))
		 || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->clienjoyholidaysv2->voyage->voyage_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->clienjoyholidaysv2->dir_output.'/voyage/'.$oldref;
				$dirdest = $conf->clienjoyholidaysv2->dir_output.'/voyage/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->clienjoyholidaysv2->dir_output.'/voyage/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->clienjoyholidaysv2->write))
		 || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->clienjoyholidaysv2->clienjoyholidaysv2_advance->validate))))
		 {
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/clienjoyholidaysv2/voyage_card.php', 1).'?id='.$this->id;
	...
	*/
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->CLIENJOYHOLIDAYSV2_VOYAGE_ADDON)) {
			$conf->global->CLIENJOYHOLIDAYSV2_VOYAGE_ADDON = 'mod_voyage_standard';
		}

		if (!empty($conf->global->CLIENJOYHOLIDAYSV2_VOYAGE_ADDON)) {
			$mybool = false;

			$file = $conf->global->CLIENJOYHOLIDAYSV2_VOYAGE_ADDON.".php";
			$classname = $conf->global->CLIENJOYHOLIDAYSV2_VOYAGE_ADDON;

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/clienjoyholidaysv2/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/clienjoyholidaysv2/doc/";
